...
class SearchHistoryManager(private val context: Context) {
    ...
    private fun getKeyForUser(userId: Int) = stringSetPreferencesKey("search_history_user_$userId")

    fun getSearchHistoryFlow(userId: Int): Flow<List<String>> = context.dataStore.data
        .map { preferences ->
            preferences[getKeyForUser(userId)]?.toList() ?: emptyList()
        }

    suspend fun addSearchItem(userId: Int, item: String) {
    ...
    suspend fun removeSearchItem(userId: Int, item: String) {
        context.dataStore.edit { preferences ->
            val currentSet = preferences[getKeyForUser(userId)] ?: emptySet()
